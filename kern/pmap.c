...
mem_init(void)
{
	...
	// Your code goes here:
	// panic("UPAGES:%x PADDR:%x\n", UPAGES, PADDR(pages));
	// pages : 11b000 size: 0x40000
	...
	// cprintf("bootstack: %x\n", PADDR(bootstack));
	// boot_map_region(kern_pgdir, KSTACKTOP-PTSIZE, PTSIZE - KSTKSIZE, PADDR(bootstack) - (PTSIZE - KSTKSIZE), PTE_W);
...
page_init(void)
{
	...
	// 0x110000 is kernel stack (bootstack)
	// 0x11c000 is kern_pgdir
	// 0x11d000 - 0x15d000 is take up by pages
	// cprintf("%x\n", PADDR(boot_alloc(0)));
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	pp->pp_ref++;
	// remove will free pp if pp_ref == 0
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	// for (; base < MMIOLIM; base += PGSIZE) {
	// 	pte_t *pte = pgdir_walk(kern_pgdir, base, 0);
	// 	if (!pte || !(*pte & PTE_P)) break;
	// }
